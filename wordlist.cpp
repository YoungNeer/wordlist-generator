...
string println (string &str,int len,string range){
  ...
  /* Aliter
    The previous one was:- (but it was a bit buggy, so i replaced it)
    string println (string &str,int len){
      int flag=0;
      for (int i=len;i>=0;--i,flag=0){
      if (str[i]=='z')
        if (i!=0){
            if (str[i-1]!='z'){
              str[i-1]+=1;
...
int main(void){
  ...
  string range,str;
  cout<<"NKEM ACADEMY WORDLIST GENRATOR\n\n";
  cout<<"1) Generate a simple wordlist\n2) Create a complex wordlist\n";
  cout<<"3) Edit a wordlist\n4) Set Default Settings\n5) Exit\n";
  cout<<"Tell me what's in your mind:> ";
  cin>>opt;
switch (opt){
case 1:
  cout<<"a) Enter the minimum no.: ";
  cin>>min;
  cout<<"b) Enter the maximum no.: ";
  cin>>max;
  cout<<"c) Enter the range: ";
  range=getRange();
  unique=getOptions("\nDo you want repetitions?\n","1. Yes\n2. No",2)-1;
